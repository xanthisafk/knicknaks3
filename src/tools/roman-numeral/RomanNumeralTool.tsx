...
export default function RomanNumeralTool() {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Panel>
          <div className="space-y-3">
            <h3 className="text-sm font-semibold text-[var(--text-primary)]">Integer → Roman</h3>
            ...
            {romanResult && (
              <div className="flex items-center gap-3">
                ...
                <button onClick={async () => { await copyToClipboard(romanResult); setCopied("rom"); setTimeout(() => setCopied(null), 1500); }} className="text-xs text-[var(--text-tertiary)] hover:text-[var(--text-primary)] cursor-pointer">{copied === "rom" ? "✓" : "Copy"}</button>
              </div>
            )}
          </div>
        </Panel>
        ...
        <Panel>
          <div className="space-y-3">
            <h3 className="text-sm font-semibold text-[var(--text-primary)]">Roman → Integer</h3>
            ...
            {numResult !== null ? (
              <div className="flex items-center gap-3">
                ...
                <button onClick={async () => { await copyToClipboard(String(numResult)); setCopied("num"); setTimeout(() => setCopied(null), 1500); }} className="text-xs text-[var(--text-tertiary)] hover:text-[var(--text-primary)] cursor-pointer">{copied === "num" ? "✓" : "Copy"}</button>
              </div>
            ) : romInput ? (
      ...
      <Panel>
        <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-3">Reference Table</h3>
