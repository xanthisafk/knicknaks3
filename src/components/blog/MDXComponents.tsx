...
export const ToolLink: React.FC<{ slug: string; name?: string }> = ({ slug, name }) => {
  return (
    <a
      href={`/tools/${slug}`}
      className="inline-flex items-center gap-1 font-semibold text-primary-500 hover:underline"
    >
...
export const Screenshot: React.FC<{ src: string; alt: string; caption?: string }> = ({ src, alt, caption }) => {
  return (
    <figure className="my-12 group">
      <div className="relative p-2 rounded-lgorder-(--border-default) bg-(--surface-elevated) shadow-md transition-shadow group-hover:shadow-xl overflow-hidden">
        <div className="doodle-border absolute inset-0 opacity-20 pointer-events-none"></div>
        <img
          src={src}
          alt={alt}
          className="h-auto w-full rounded-md relative z-10"
          loading="lazy"
        />
      </div>
      {caption && (
        <figcaption className="mt-4 text-center text-sm font-medium italic text-(--text-secondary) flex items-center justify-center gap-2">
          <span className="text-primary-500">✎</span> {caption}
        </figcaption>
      )}
    </figure>
  );
...
export const ComparisonTable: React.FC<{ headers: string[]; rows: (string | boolean)[][] }> = ({ headers, rows }) => {
  return (
    <div className="my-10 overflow-hidden rounded-xl border-2 border-(--border-default) shadow-sm bg-(--surface-bg) relative">
